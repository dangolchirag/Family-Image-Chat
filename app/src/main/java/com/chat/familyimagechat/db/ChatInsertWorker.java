...
//        Log.i(TAG, "ChatInsertWorker: "+localChatSourceRepository);
    ...
    public Result doWork() {
        ...
        try {
            ...
            FamilyChatEntity chatItem = new FamilyChatEntity(chatId, System.currentTimeMillis(), true, MessageDelivery.DELIVERED);
            localChatSourceRepository.upsertChat(chatItem);
            Log.i(TAG, "doWork: "+localChatSourceRepository);
            return Result.success();  // Return success if everything is okay
        } catch (Exception e) {
            e.printStackTrace();
